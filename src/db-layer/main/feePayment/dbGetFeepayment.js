...
class DbGetFeepaymentCommand extends DBGetMongooseCommand {
  constructor(input) {
    super(input, FeePayment);
    this.commandName = "dbGetFeepayment";
...
const dbGetFeepayment = (input) => {
  input.id = input.feePaymentId;
  const dbGetCommand = new DbGetFeepaymentCommand(input);
  return dbGetCommand.execute();
};

module.exports = dbGetFeepayment;
