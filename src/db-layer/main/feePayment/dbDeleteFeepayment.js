...
class DbDeleteFeepaymentCommand extends DBSoftDeleteMongooseCommand {
  constructor(input) {
    const instanceMode = true;
    super(input, FeePayment, instanceMode);
    this.commandName = "dbDeleteFeepayment";
...
const dbDeleteFeepayment = async (input) => {
  input.id = input.feePaymentId;
  const dbDeleteCommand = new DbDeleteFeepaymentCommand(input);
  return dbDeleteCommand.execute();
};

module.exports = dbDeleteFeepayment;
