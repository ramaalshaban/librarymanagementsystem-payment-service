...
class DbListFeepaymentsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListFeepayments";
    this.emptyResult = true;
    this.objectName = "feePayments";
  ...
  async transposeResult() {
    for (const feePayment of this.dbData.items) {
  ...
  createQuery() {
    ...
    return FeePayment.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (FeePayment.getCqrsJoins) {
      await FeePayment.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await FeePayment.countDocuments(
      this.whereClause,
    );
  }
...
const dbListFeepayments = (input) => {
  const dbGetListCommand = new DbListFeepaymentsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListFeepayments;
