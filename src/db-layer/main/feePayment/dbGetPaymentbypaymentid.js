...
class DbGetPaymentbypaymentidCommand extends DBGetMongooseCommand {
  constructor(input) {
    super(input, FeePayment);
    this.commandName = "dbGetPaymentbypaymentid";
...
const dbGetPaymentbypaymentid = (input) => {
  input.id = input.feePaymentId;
  const dbGetCommand = new DbGetPaymentbypaymentidCommand(input);
  return dbGetCommand.execute();
};

module.exports = dbGetPaymentbypaymentid;
