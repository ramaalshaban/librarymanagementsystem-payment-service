...
const deleteFeePaymentById = async (id) => {
  try {
    if (typeof id === "object") {
      id = id.id;
    }
    ...
    const existingDoc = await FeePayment.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const deletedDoc = await FeePayment.findOneAndUpdate(
...
module.exports = deleteFeePaymentById;
