...
const getFeePaymentStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(FeePayment.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            FeePayment.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await FeePayment.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingFeePaymentStatsByQuery",
      err,
    );
  }
};

module.exports = getFeePaymentStatsByQuery;
