...
class DbUpdateFeeCommand extends DBUpdateMongooseCommand {
  constructor(input) {
    const instanceMode = true;
    input.isBulk = false;
    input.updateEach = false;
    super(input, Fee, instanceMode);
    this.commandName = "dbUpdateFee";
...
const dbUpdateFee = async (input) => {
  input.id = input.feeId;
  const dbUpdateCommand = new DbUpdateFeeCommand(input);
  return await dbUpdateCommand.execute();
};

module.exports = dbUpdateFee;
