...
class DbCheckoutrefreshFeeCommand extends DBUpdateMongooseCommand {
  constructor(input) {
    const instanceMode = true;
    input.isBulk = false;
    input.updateEach = false;
    super(input, Fee, instanceMode);
    this.commandName = "dbCheckoutrefreshFee";
...
const dbCheckoutrefreshFee = async (input) => {
  input.id = input.feeId;
  const dbUpdateCommand = new DbCheckoutrefreshFeeCommand(input);
  return await dbUpdateCommand.execute();
};

module.exports = dbCheckoutrefreshFee;
