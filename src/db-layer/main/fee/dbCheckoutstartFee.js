...
class DbCheckoutstartFeeCommand extends DBUpdateMongooseCommand {
  constructor(input) {
    const instanceMode = true;
    input.isBulk = false;
    input.updateEach = false;
    super(input, Fee, instanceMode);
    this.commandName = "dbCheckoutstartFee";
...
const dbCheckoutstartFee = async (input) => {
  input.id = input.feeId;
  const dbUpdateCommand = new DbCheckoutstartFeeCommand(input);
  return await dbUpdateCommand.execute();
};

module.exports = dbCheckoutstartFee;
