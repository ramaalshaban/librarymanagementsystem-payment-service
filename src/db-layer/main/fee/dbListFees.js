...
class DbListFeesCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListFees";
    this.emptyResult = true;
    this.objectName = "fees";
  ...
  async transposeResult() {
    for (const fee of this.dbData.items) {
  ...
  createQuery() {
    ...
    return Fee.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (Fee.getCqrsJoins) {
      await Fee.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await Fee.countDocuments(this.whereClause);
  }
...
const dbListFees = (input) => {
  const dbGetListCommand = new DbListFeesCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListFees;
