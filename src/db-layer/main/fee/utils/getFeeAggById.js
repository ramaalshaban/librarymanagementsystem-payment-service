...
const getFeeAggById = async (feeId) => {
  try {
    let feeQuery;
    ...
    const fee = await feeQuery.exec();

    if (!fee) {
      return null;
    }
    const feeData =
      Array.isArray(feeId) && feeId.length > 0
        ? fee.map((item) => item.getData())
        : fee.getData();

    // should i add this here?
    await Fee.getCqrsJoins(feeData);

    return feeData;
  } catch (err) {
    console.log(err);
    throw new HttpServerError("errMsg_dbErrorWhenRequestingFeeAggById", err);
  }
};
