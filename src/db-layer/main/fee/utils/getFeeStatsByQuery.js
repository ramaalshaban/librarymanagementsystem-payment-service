...
const getFeeStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Fee.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            Fee.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await Fee.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingFeeStatsByQuery",
      err,
    );
  }
};

module.exports = getFeeStatsByQuery;
