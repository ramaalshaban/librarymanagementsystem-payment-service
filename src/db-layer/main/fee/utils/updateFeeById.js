...
const updateFeeById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Fee.findOne({ _id: id, isActive: true });
    ...
    const dbDoc = await Fee.findOneAndUpdate(whereClause, dataClause, options);
    ...
    throw new HttpServerError("errMsg_dbErrorWhenUpdatingFeeById", err);
  }
};

module.exports = updateFeeById;
