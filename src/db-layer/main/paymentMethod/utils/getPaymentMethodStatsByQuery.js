...
const getPaymentMethodStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(PaymentMethod.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            PaymentMethod.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await PaymentMethod.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingPaymentMethodStatsByQuery",
      err,
    );
  }
};

module.exports = getPaymentMethodStatsByQuery;
