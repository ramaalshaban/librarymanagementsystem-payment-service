...
class DbDoListethodsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbDoListethods";
    this.emptyResult = true;
    this.objectName = "paymentMethods";
  ...
  async transposeResult() {
    for (const paymentMethod of this.dbData.items) {
  ...
  createQuery() {
    ...
    return PaymentMethod.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (PaymentMethod.getCqrsJoins) {
      await PaymentMethod.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await PaymentMethod.countDocuments(
      this.whereClause,
    );
  }
...
const dbDoListethods = (input) => {
  const dbGetListCommand = new DbDoListethodsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbDoListethods;
