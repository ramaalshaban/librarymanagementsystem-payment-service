...
class DbListFeeeventsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListFeeevents";
    this.emptyResult = true;
    this.objectName = "feeEvents";
  ...
  async transposeResult() {
    for (const feeEvent of this.dbData.items) {
  ...
  createQuery() {
    ...
    return FeeEvent.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (FeeEvent.getCqrsJoins) {
      await FeeEvent.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await FeeEvent.countDocuments(
      this.whereClause,
    );
  }
...
const dbListFeeevents = (input) => {
  const dbGetListCommand = new DbListFeeeventsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListFeeevents;
