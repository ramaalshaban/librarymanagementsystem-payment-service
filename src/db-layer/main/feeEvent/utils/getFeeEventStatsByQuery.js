...
const getFeeEventStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(FeeEvent.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            FeeEvent.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await FeeEvent.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingFeeEventStatsByQuery",
      err,
    );
  }
};

module.exports = getFeeEventStatsByQuery;
