...
const getPaymentCustomerStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(PaymentCustomer.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            PaymentCustomer.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await PaymentCustomer.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingPaymentCustomerStatsByQuery",
      err,
    );
  }
};

module.exports = getPaymentCustomerStatsByQuery;
