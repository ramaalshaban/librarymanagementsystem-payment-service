...
class DbListCustomersCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListCustomers";
    this.emptyResult = true;
    this.objectName = "paymentCustomers";
  ...
  async transposeResult() {
    for (const paymentCustomer of this.dbData.items) {
  ...
  createQuery() {
    ...
    return PaymentCustomer.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (PaymentCustomer.getCqrsJoins) {
      await PaymentCustomer.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await PaymentCustomer.countDocuments(
      this.whereClause,
    );
  }
...
const dbListCustomers = (input) => {
  const dbGetListCommand = new DbListCustomersCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListCustomers;
