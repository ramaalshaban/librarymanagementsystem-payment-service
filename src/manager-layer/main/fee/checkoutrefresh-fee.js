...
const { dbCheckoutrefreshFee } = require("dbLayer");

class CheckoutrefreshFeeManager extends FeeManager {
  ...
  async doBusiness() {
    // Call DbFunction
    // make an awaited call to the dbCheckoutrefreshFee function to update the fee and return the result to the controller
    const fee = await dbCheckoutrefreshFee(this);

    return fee;
  }

  async raiseEvent() {
    FeeCheckoutrefreshkedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = CheckoutrefreshFeeManager;
