...
class DeleteFeeMcpController extends PaymentMcpController {
  constructor(params) {
    super("deleteFee", "deletefee", params);
    this.dataName = "fee";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteFeeManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deleteFee",
    description:
      "Delete a fee/order record. Used with care (admin/maintenance only) as this affects audit trail.",
    parameters: DeleteFeeMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteFeeMcpController = new DeleteFeeMcpController(mcpParams);
      try {
        const result = await deleteFeeMcpController.processRequest();
        //return DeleteFeeMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
