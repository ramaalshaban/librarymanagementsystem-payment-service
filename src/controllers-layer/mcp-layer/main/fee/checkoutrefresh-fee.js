...
class CheckoutrefreshFeeMcpController extends PaymentMcpController {
  constructor(params) {
    super("checkoutrefreshFee", "checkoutrefreshfee", params);
    this.dataName = "fee";
    this.crudType = "update";
  }

  createApiManager() {
    return new CheckoutrefreshFeeManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description: "",
    parameters: CheckoutrefreshFeeMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const checkoutrefreshFeeMcpController =
        new CheckoutrefreshFeeMcpController(mcpParams);
      try {
        const result = await checkoutrefreshFeeMcpController.processRequest();
        //return CheckoutrefreshFeeMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
