...
class CheckoutstartFeeMcpController extends PaymentMcpController {
  constructor(params) {
    super("checkoutstartFee", "checkoutstartfee", params);
    this.dataName = "fee";
    this.crudType = "update";
  }

  createApiManager() {
    return new CheckoutstartFeeManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description: "",
    parameters: CheckoutstartFeeMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const checkoutstartFeeMcpController = new CheckoutstartFeeMcpController(
        mcpParams,
      );
      try {
        const result = await checkoutstartFeeMcpController.processRequest();
        //return CheckoutstartFeeMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
