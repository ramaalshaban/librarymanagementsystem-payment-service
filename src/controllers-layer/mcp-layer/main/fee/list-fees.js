...
class ListFeesMcpController extends PaymentMcpController {
  constructor(params) {
    super("listFees", "listfees", params);
    this.dataName = "fees";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListFeesManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "listFees",
    description:
      "Get a list of library fee/order records, filterable by userId, loanId, status, or other attributes.",
    parameters: ListFeesMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listFeesMcpController = new ListFeesMcpController(mcpParams);
      try {
        const result = await listFeesMcpController.processRequest();
        //return ListFeesMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
