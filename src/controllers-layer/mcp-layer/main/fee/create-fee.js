...
class CreateFeeMcpController extends PaymentMcpController {
  constructor(params) {
    super("createFee", "createfee", params);
    this.dataName = "fee";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateFeeManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "createFee",
    description:
      "Create a new overdue or lost/damage fee entry for a member. May be created by system or staff.",
    parameters: CreateFeeMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const createFeeMcpController = new CreateFeeMcpController(mcpParams);
      try {
        const result = await createFeeMcpController.processRequest();
        //return CreateFeeMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
