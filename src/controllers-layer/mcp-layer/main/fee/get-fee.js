...
class GetFeeMcpController extends PaymentMcpController {
  constructor(params) {
    super("getFee", "getfee", params);
    this.dataName = "fee";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetFeeManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetFeeMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getFeeMcpController = new GetFeeMcpController(mcpParams);
      try {
        const result = await getFeeMcpController.processRequest();
        //return GetFeeMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
