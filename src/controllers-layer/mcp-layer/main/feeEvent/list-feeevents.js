...
class ListFeeEventsMcpController extends PaymentMcpController {
  constructor(params) {
    super("listFeeEvents", "listfeeevents", params);
    this.dataName = "feeEvents";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListFeeEventsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "listFeeEvents",
    description:
      "Get a list of fee event logs, filterable by feeId, eventType, actorUserId.",
    parameters: ListFeeEventsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listFeeEventsMcpController = new ListFeeEventsMcpController(
        mcpParams,
      );
      try {
        const result = await listFeeEventsMcpController.processRequest();
        //return ListFeeEventsMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
