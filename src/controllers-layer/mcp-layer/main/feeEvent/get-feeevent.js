...
class GetFeeEventMcpController extends PaymentMcpController {
  constructor(params) {
    super("getFeeEvent", "getfeeevent", params);
    this.dataName = "feeEvent";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetFeeEventManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetFeeEventMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getFeeEventMcpController = new GetFeeEventMcpController(mcpParams);
      try {
        const result = await getFeeEventMcpController.processRequest();
        //return GetFeeEventMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
