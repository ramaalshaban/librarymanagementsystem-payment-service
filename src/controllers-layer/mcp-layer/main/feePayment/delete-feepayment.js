...
class DeleteFeePaymentMcpController extends PaymentMcpController {
  constructor(params) {
    super("deleteFeePayment", "deletefeepayment", params);
    this.dataName = "feePayment";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteFeePaymentManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deleteFeePayment",
    description: "Delete a fee payment (admin/maintenance only).",
    parameters: DeleteFeePaymentMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteFeePaymentMcpController = new DeleteFeePaymentMcpController(
        mcpParams,
      );
      try {
        const result = await deleteFeePaymentMcpController.processRequest();
        //return DeleteFeePaymentMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
