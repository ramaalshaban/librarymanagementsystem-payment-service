...
class GetFeePaymentMcpController extends PaymentMcpController {
  constructor(params) {
    super("getFeePayment", "getfeepayment", params);
    this.dataName = "feePayment";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetFeePaymentManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetFeePaymentMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getFeePaymentMcpController = new GetFeePaymentMcpController(
        mcpParams,
      );
      try {
        const result = await getFeePaymentMcpController.processRequest();
        //return GetFeePaymentMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
