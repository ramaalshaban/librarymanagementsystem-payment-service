...
app.use("", feeRouter);
app.use("", feePaymentRouter);
app.use("", feeEventRouter);
app.use("", feePaymentRouter);
app.use("", paymentCustomerRouter);
...
const { getFeeController } = require("./utils/checkout-controllers");

app.use(
  "/stripe-demo/fee/images",
  express.static(path.join(__dirname, "stripe-demo", "fee", "images")),
);
app.use(
  "/stripe-demo/fee/lib",
  express.static(path.join(__dirname, "stripe-demo", "fee", "lib")),
);

const { getFeeStripeDemoController } = require("./utils/checkout-controllers");
app.get("/stripe-demo/fee/fetch-created-order", getFeeController);
app.get("/stripe-demo/fee", getFeeStripeDemoController);
app.get("/stripe-demo/fee/*name", getFeeStripeDemoController);

const {
  getPaymentMethodsController,
  addPaymentMethodToCustomerController,
  deletePaymentMethodController,
} = require("./utils/checkout-controllers");
app.get("/user-payment-methods", getPaymentMethodsController);
app.post("/add-payment-method", addPaymentMethodToCustomerController);
app.delete(
...
app.get("/getPostmanCollection", (req, res) => {
  const filePath = path.join(
    __dirname,
    "librarymanagementsystem-payment-postman-collection.json",
  );
  console.log("Downloading Postman Collection from:", filePath);
  res.setHeader("Content-Type", "application/json");
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="librarymanagementsystem-payment-postman-collection.json"',
  );
