...
describe("GetCustomerByUserId", () => {
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/PaymentCustomer/get-customerbyuserid",
      {
        "./PaymentCustomerManager": class {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("getCustomerByUserId");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("get");
    expect(instance.dataName).to.equal("paymentCustomer");
  });
});
